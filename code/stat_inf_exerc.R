...
# ver.1 (inspired by the previous simulation)
nosim <- 1000
mu <- .5; sd <- sqrt(mu * (1 - mu))
cfunc <- function(x, n) sqrt(n) * (mean(x) - mu) / sd
dat <- data.frame(
    x = c(apply(matrix(sample(0:1, nosim*10, replace=T), nosim), 1, cfunc, 10),
          apply(matrix(sample(0:1, nosim*20, replace=T), nosim), 1, cfunc, 20),
          apply(matrix(sample(0:1, nosim*30, replace=T), nosim), 1, cfunc, 30)),
    size = factor(rep(c(10,20,30), rep(nosim, 3))))
require(ggplot2)
